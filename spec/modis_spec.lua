...
describe('modis', function()
  ...
  describe('Collection', function()
    ...
    describe(':size', function()
      it('starts at 0', function()
        assert.equals(0, db.users:find():size())
      end)
      it('can count with a query', function()
        db.users:insert({{a=1}, {a=2}, {a=3}})
        assert.equals(2, db.users:find({a = {['$lt'] = 3}}):size())
      end)
      it('respects skip & limit', function()
        db.users:insert({{a=1}, {a=2}, {a=3}})
        assert.equals(1, db.users:find():limit(2):skip(1):size())
      end)
    end)

    describe(':count', function()
      it('starts at 0', function()
        assert.equals(0, db.users:find():count())
      end)
      it('can count with a query', function()
        db.users:insert({{a=1}, {a=2}, {a=3}})
        assert.equals(2, db.users:find({a = {['$lt'] = 3}}):count())
      end)
      it('only respects skip & limit if true is passed', function()
        db.users:insert({{a=1}, {a=2}, {a=3}})
        assert.equals(3, db.users:find():limit(2):skip(1):count())
        assert.equals(1, db.users:find():limit(2):skip(1):count(true))
      end)
    end)
    ...
    describe(':insert', function()
      ...
      it('keeps track of the max id', function()
        db.users:insert({name = 'harvey', _id = 3})
        assert.same(db.users:find({_id = 3}):toArray(), {{name = 'harvey', _id = 3}})
        db.users:insert({name = 'laurent'})
        assert.same(db.users:findOne({name = 'laurent'}), {name = 'laurent', _id = 4})
      end)
    ...
    describe(':find', function()
      it('returns all elements when the query is empty', function()
        ...
        assert.equals(db.users:find({}):count(), 3)
      end)
  ...
  describe('Cursor', function()
    describe(':toArray()', function()
      ...
      it('returns as many elements as the query ', function()
        db.vowels:insert({{a=1},{e=1},{i=1}})
        assert.same(db.vowels:find():toArray(), {{a=1,_id=1},{e=1,_id=2},{i=1,_id=3}})
      end)
    end)
    describe(':forEach', function()
      it('executes the given callback on each document of the cursor', function()
        db.seasons:insert({{n='summer'},{n='autumn'},{n='winter'},{n='spring'}})
        local names = {}
        db.seasons:find():forEach(function(doc) names[#names + 1] = doc.n end)
        assert.same(names, {'summer', 'autumn', 'winter', 'spring'})
      end)
    end)
    describe(':map', function()
      it('executes the given callback on each document of the cursor, concatenating the results', function()
        db.seasons:insert({{n='summer'},{n='autumn'},{n='winter'},{n='spring'}})
        assert.same(db.seasons:find():map(function(doc) return doc.n end), {'summer', 'autumn', 'winter', 'spring'})
      end)
    end)
    describe('[<integer>]', function()
      it('returns nil if the collection is empty', function()
        assert.is_nil(db.people:find()[1])
      end)
      it('returns the n-esim document of the sequence', function()
        db.workdays:insert({{n='monday'},{n='tuesday'},{n='aurelian friday'}})

        assert.equal(db.workdays:find()[1].n, 'monday')
        assert.equal(db.workdays:find()[2].n, 'tuesday')
      end)
      it('respects the limit', function()
        db.workdays:insert({{n='monday'},{n='tuesday'},{n='aurelian friday'}})
        assert.is_nil(db.workdays:find():limit(1)[2])
      end)
    end)
    describe(':next', function()
      it('returns nil when there are no elements', function()
        assert.is_nil(db.cats:find():next())
      end)
      it('returns the first element, then the second, etc', function()
        db.numbers:insert({{one = 1}, {two = 2}, {three = 3}})
        local c = db.numbers:find()
        assert.same(c:next(), {one   = 1, _id = 1})
        assert.same(c:next(), {two   = 2, _id = 2})
        assert.same(c:next(), {three = 3, _id = 3})
        assert.is_nil(c:next())
      end)
      it('respects the limit', function()
        db.numbers:insert({{one = 1}, {two = 2}, {three = 3}})
        local c = db.numbers:find():limit(2)
        assert.same(c:next(), {one   = 1, _id = 1})
        assert.same(c:next(), {two   = 2, _id = 2})
        assert.is_nil(c:next())
      end)
    ...
    describe(':hasNext', function()
      it('returns false when there are no elements', function()
        assert.is_false(db.cats:find():hasNext())
      end)
      it('returns true when there are elements left', function()
        db.numbers:insert({{one = 1}, {two = 2}, {three = 3}})
        local c = db.numbers:find()
        assert.is_true(c:hasNext())
        c:next()
        assert.is_true(c:hasNext())
        c:next()
        assert.is_true(c:hasNext())
        c:next()
        assert.is_false(c:hasNext())
      end)
      it('respects the limits', function()
        db.numbers:insert({{one = 1}, {two = 2}, {three = 3}})
        local c = db.numbers:find():limit(2)
        assert.is_true(c:hasNext())
        c:next()
    ...
    describe(':skip', function()
      it('advances the start', function()
        db.numbers:insert({{one = 1}, {two = 2}, {three = 3}})
        local c = db.numbers:find():skip(2)
        assert.is_true(c:hasNext())
        c:next()
        assert.is_false(c:hasNext())
      end)
      it('respects the limits', function()
        db.numbers:insert({{one = 1}, {two = 2}, {three = 3}})
        local c = db.numbers:find():limit(2):skip(1)
        assert.is_true(c:hasNext())
        c:next()
        assert.is_false(c:hasNext())
      end)
